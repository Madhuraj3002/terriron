...
<table id="myTable">
  <tr class="header">
	...
	<th style="width:60%;">Total Amount Received from admin</th>
...
while($row=mysqli_fetch_array($query))
{
    $sid = $row['id'];
			...
			$query=mysqli_query($connect,"SELECT * FROM `transaction` WHERE `sid`='$sid' ") or die(mysqli_error($connect));
                             $x=1;
while($row=mysqli_fetch_array($query))
{
	$gname=$row['gname'];
	...
	$sname=$row['uname'];
    $semail=$row['uemail'];
	$scontact=$row['ucontact'];
	$saddress=$row['uaddress'];

	$quantity=$row['quantity'];
	$totalamt=$row['useramt'];
	$date=$row['date'];
