...
$query=mysqli_query($connect,"SELECT `name` FROM `added_items` WHERE `name`='$name' && `supplier_id`='$sid' ") or die(mysqli_error($connect));
$row=mysqli_fetch_array($query);
$nid=$row['name'];
...
if($nid==NULL)
{
 ...
 $query = "INSERT INTO `added_items` (`name`, `price`, `quantity`, `supplier_id`) VALUES ('$name','$price','$kg','$sid')";

 if(mysqli_query($connect, $query))  
      {  
   
print '<script>alert("Agri Products Item uploaded successfully  ,Thankyou");</script>';
	  }
	}
	else{
print '<script>alert("Agri Products Item Already exists, Please edit it in edit section  ,Thankyou");</script>';
...
<form method="POST" action="items.php">
 ...
 <h3 align="center" color="red">Add Agri products</h3>  	 
	 </head>  
      <body>  
