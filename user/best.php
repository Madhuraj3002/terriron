...
$Agri_Products = "Brown Rice";
if(isset($_GET['search']))
{
	if(isset($_GET['name']))
{
	$Agri_Products=$_GET['name'];
...
<center>
<h2>Agri_Products Items</h2>
</center>
<center>

<input type="text" id="myInput" onkeyup="myFunction()" placeholder="Search by Agri_Products names.." title="Type in a name">
<div style="overflow-x:auto;">

<table id="myTable">
  <tr class="header">
    <th style="width:60%;">Agri_Products Name</th>
    ...
    <th style="width:60%;">Buy</th>
		 ...
		 $user="root";
				...
				$connect=new mysqli("localhost",$user,$password,$db) or die("not connected");
				echo $Agri_Products;
			//$query=mysqli_query($connect,"SELECT * FROM `added_items` WHERE `name` = '$Agri_Products' & MIN(price) ") or die(mysqli_error($connect));
			 $query=mysqli_query($connect,"SELECT `id`, `name`, `price`, `quantity`, `supplier_id` FROM `added_items` a WHERE price = (SELECT MIN(`price`) FROM `added_items` b WHERE b.name = a.name)") or die(mysqli_error($connect));
           
           // $query=mysqli_query($connect,"SELECT `name`, MIN(`price`), `quantity`, `supplier_id` FROM `added_items` WHERE `name` = '$Agri_Products'") or die(mysqli_error($connect));
                             
			//				 "SELECT * FROM `added_items` WHERE `name` = '$Agri_Products' & `price` IN (SELECT MIN(`price`) FROM `added_items`)"
							 $x=1;
